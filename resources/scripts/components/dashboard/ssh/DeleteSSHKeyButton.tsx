...
export default ({ name, fingerprint }: { name: string; fingerprint: string }) => {
    ...
    const onClick = () => {
        ...
        Promise.all([mutate((data) => data?.filter((value) => value.fingerprint !== fingerprint), false), deleteSSHKey(fingerprint)]).catch((error) => {
    ...
    return (
        <>
            <Dialog.Confirm open={visible} title={'Delete SSH Key'} confirm={'Delete Key'} onConfirmed={onClick} onClose={() => setVisible(false)}>
