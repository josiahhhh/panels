...
export default ({ server, className }: { server: Server; className?: string }) => {
    ...
    return (
        <StatusIndicatorBox
            ...
            $status={stats?.status}
            style={{
                backgroundImage: `url(https://cdn.iceline.host/img/headers/game-panel/${server.description}.jpg)`,
                backgroundSize: 'cover',
                backgroundRepeat: 'no-repeat',
            }}
        >
            <div css={tw`flex items-center col-span-8 sm:col-span-5 lg:col-span-4`}>
                <div css={tw`mr-4`}>
                    {server.description === 'minecraft' && <img src='https://cdn.iceline.host/img/game-servers/minecraft.png' alt='Minecraft' />}
                    {server.description === 'gmod' && <img src='https://cdn.iceline.host/img/game-servers/gmod.png' alt="Garry's Mod" />}
                </div>
                <div>
                    <p css={tw`text-lg break-words`}>{server.name}</p>
                    {!!server.description && <p css={tw`text-sm text-neutral-200 break-words line-clamp-2`}>{server.description}</p>}
                </div>
            </div>
            <div css={tw`hidden lg:col-span-4 lg:flex ml-4 justify-end h-full`}>
                ...
                {!stats || isSuspended ? (
                    isSuspended ? (
                        <div css={tw`flex-1 text-center`}>
                            <span css={tw`bg-red-500 rounded px-2 py-1 text-red-100 text-xs`}>{server.status === 'suspended' ? 'Suspended' : 'Connection Error'}</span>
                        </div>
                    ) : server.isTransferring || server.status ? (
                        <div css={tw`flex-1 text-center`}>
                            <span css={tw`bg-neutral-500 rounded px-2 py-1 text-neutral-200 text-xs`}>
