...
const ConfirmationModal: React.FC<Props> = ({ title, children, buttonText, onConfirmed, showSpinnerOverlay }) => {
        ...
        <>
            ...
            <div css={tw`flex flex-wrap items-center justify-end mt-8`}>
                ...
                <Button color={'red'} css={tw`w-full sm:w-auto mt-4 sm:mt-0 sm:ml-4`} onClick={() => onConfirmed()}>
                    {!showSpinnerOverlay ? <span>{buttonText}</span> : <Spinner size={'small'} />}
                </Button>
            </div>
        </>
    );
