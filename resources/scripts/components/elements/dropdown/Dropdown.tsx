...
const DropdownGap = ({ invisible }: { invisible?: boolean }) => <div className={classNames('border m-2', { 'border-neutral-700': !invisible, 'border-transparent': invisible })} />;
...
const Dropdown = forwardRef<typeof Menu, Props>(({ as, children }, ref) => {
    const [Button, items] = useMemo(() => {
        ...
        return [list.filter((child) => child.type === DropdownButton), list.filter((child) => child.type !== DropdownButton)];
    }, [children]);
