...
const ServerDetailsBlock = ({ className }: { className?: string }) => {
    ...
    return (
        <div className={classNames('grid grid-cols-6 gap-2 md:gap-4', className)}>
            ...
            <StatBlock icon={faClock} title={'Uptime'} color={getBackgroundColor(status === 'running' ? 0 : status !== 'offline' ? 9 : 10, 10)}>
            ...
            <StatBlock icon={faMicrochip} title={'CPU Load'} color={getBackgroundColor(stats.cpu, limits.cpu)}>
                {status === 'offline' ? <span className={'text-gray-400'}>Offline</span> : <Limit limit={textLimits.cpu}>{stats.cpu.toFixed(2)}%</Limit>}
            </StatBlock>
            <StatBlock icon={faMemory} title={'Memory'} color={getBackgroundColor(stats.memory / 1024, limits.memory * 1024)}>
                {status === 'offline' ? <span className={'text-gray-400'}>Offline</span> : <Limit limit={textLimits.memory}>{bytesToString(stats.memory)}</Limit>}
            </StatBlock>
