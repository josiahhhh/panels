...
const AllocationRow = ({ allocation, locked }: Props) => {
        ...
        <GreyRowBox $hoverable={false} className={'flex-wrap md:flex-nowrap mt-2'}>
            ...
            <div className={'mt-4 w-full md:mt-0 md:flex-1 md:w-auto'}>
                <InputSpinner visible={loading}>
                    <Textarea
                        className={'bg-icelinemainbackground-600 hover:border-icelinebrandcolour-500 border-transparent'}
                        placeholder={'Notes'}
                        defaultValue={allocation.notes || undefined}
                        onChange={(e) => (locked ? setAllocationNotes(e.currentTarget.value) : null)}
                        disabled={locked}
                    />
                </InputSpinner>
            </div>
