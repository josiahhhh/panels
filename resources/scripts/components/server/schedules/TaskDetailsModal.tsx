...
import DatabaseValues from '@/components/server/schedules/DatabaseValues';
...
const schema = object().shape({
    action: string().required().oneOf(['command', 'power', 'backup', 'database-backup', 'wipe']),
            ...
            {({ isSubmitting, values }) => (
                <Form css={tw`m-0`}>
                        ...
                        <div css={tw`mr-2 w-1/3`}>
                            ...
                            <FormikFieldWrapper name={'action'}>
                                <FormikField as={Select} name={'action'}>
                                    ...
                                    <option value={'database-backup'}>Create database backup</option>
                                    {eggName.name.includes('Rust') && <option value={'wipe'}>Wipe server</option>}
                                </FormikField>
                            </FormikFieldWrapper>
                        </div>
                    ...
                    <div css={tw`mt-6`}>
                        ...
                        ) : values.action === 'database-backup' ? (
                            <div>
                                <Label>Database Backup Parameters</Label>
                                <FormikFieldWrapper name={'payload'}>
                                    <FormikField component={DatabaseValues} name={'payload'} />
                                </FormikFieldWrapper>
                            </div>
                        ) : values.action === 'wipe' ? (
                            <div>
                    ...
                    <div css={tw`mt-6 bg-neutral-700 border border-neutral-800 shadow-inner p-4 rounded`}>
                        <FormikSwitch name={'continueOnFailure'} description={'Future tasks will be run when this task fails.'} label={'Continue on Failure'} />
                    </div>
