...
import { faArrowCircleDown, faClock, faCode, faFileArchive, faPencilAlt, faToggleOn, faTrashAlt, faSpinner } from '@fortawesome/free-solid-svg-icons';
...
const getActionDetails = (action: string): [string, any] => {
    switch (action) {
        ...
        case 'database-backup':
            return ['Create Database Backup', faFileArchive];
        case 'wipe':
            return ['Wipe Server', faSpinner];
        default:
...
export default ({ schedule, task }: Props) => {
    ...
    return (
        <div css={tw`sm:flex items-center p-3 sm:p-6 bg-icelinebox-500 border-b border-icelinebox-800`}>
            <SpinnerOverlay visible={isLoading} fixed size={'large'} />
            <TaskDetailsModal schedule={schedule} task={task} visible={isEditing} onModalDismissed={() => setIsEditing(false)} />
            <ConfirmationModal
            ...
            <div css={tw`flex-none sm:flex-1 w-full sm:w-auto overflow-x-auto`}>
                ...
                {task.payload && (
                    <div css={tw`md:ml-6 mt-2`}>
                        {task.action === 'backup' && <p css={tw`text-xs uppercase text-neutral-400 mb-1`}>Ignoring files & folders:</p>}
                        <div css={tw`font-mono bg-icelinebox-700 rounded py-1 px-2 text-sm w-auto inline-block whitespace-pre-wrap break-all`}>{task.payload}</div>
                    </div>
                )}
            </div>
