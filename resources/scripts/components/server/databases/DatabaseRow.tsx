...
export default ({ database, className }: Props) => {
    ...
    const jdbcConnectionString = `jdbc:mysql://${database.username}${database.password ? `:${database.password}` : ''}@${database.connectionString}/${database.name}`;
        ...
        <>
            <Formik onSubmit={submit} initialValues={{ confirm: '' }} validationSchema={schema} isInitialValid={false}>
                {({ isSubmitting, isValid, resetForm }) => (
                    ...
                    >
                        ...
                        <p css={tw`text-sm`}>
                            Deleting a database is a permanent action, it cannot be undone. This will permanently delete the <strong>{database.name}</strong> database and remove
                            all associated data.
                        </p>
            ...
            <GreyRowBox $hoverable={false} className={className} css={tw`mb-2`}>
                ...
                <div css={tw`ml-8 text-center hidden md:block`}>
                    <CopyOnClick text={database.connectionString}>
                        <p css={tw`text-sm`}>{database.connectionString.split(':')[0]}</p>
                    </CopyOnClick>
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>Host</p>
                </div>
                <div css={tw`ml-8 text-center hidden md:block`}>
                    <p css={tw`text-sm`}>3306</p>
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>Port</p>
                </div>
                <div css={tw`hidden`}>
                ...
                <div css={tw`ml-8 text-center hidden md:block`}>
                    <Button isSecondary css={tw`border-icelinebox-500`}>
                        <p css={tw`text-sm`}>
                            {passwordVisible ? (
                                <CopyOnClick text={database.password}>
                                    <p css={tw`text-sm`}>{database.password}</p>
                                </CopyOnClick>
                            ) : (
                                <p css={tw`text-sm`} onClick={() => setPasswordVisible(!passwordVisible)}>
                                    *********
                                </p>
                            )}
                        </p>
                        <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`} onClick={() => setPasswordVisible(!passwordVisible)}>
                            Password
                        </p>
                    </Button>
                </div>
                <div css={tw`ml-8`}>
                    <Button
                        ...
                        onClick={(e) => {
                            e.preventDefault();
                            window.open(`https://${database.connectionString.split(':')[0]}/index.php?pma_username=${database.username}`, '_blank');
                        }}
                    >
                        <FontAwesomeIcon icon={faDatabase} fixedWidth />
                    </Button>
