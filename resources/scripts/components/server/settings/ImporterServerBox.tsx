...
const ImporterContainer = () => {
    ...
    const submit = (values: Values) => {
        ...
        importer(uuid, values.user, values.password, values.hote, values.port, values.srcdestination, values.dstdestination, wipe, type)
            .catch((error) => {
        ...
        <>
                ...
                >
                    <Form css={tw`mt-2 flex grid grid-cols-1 md:grid-cols-2`}>
                        <div css={tw`mt-6 mx-2`}>
                            <Field id={'user'} name={'user'} label={'User'} type={'text'} />
                        </div>
                        <div css={tw`mt-6 mx-2`}>
                            <Field id={'password'} name={'password'} type={'password'} label={'Password'} />
                        </div>
                        <div css={tw`mt-6 mx-2`}>
                            <Field id={'hote'} name={'hote'} label={'Server ip'} type={'text'} />
                        </div>
                        <div css={tw`mt-6 mx-2`}>
                            <Field id={'port'} name={'port'} label={'Server Port'} type={'text'} />
                        </div>
                        <div css={tw`mt-6 mx-2`}>
                            <Field id={'srcdestination'} name={'srcdestination'} label={'Source folder'} description={'The sftp folder where files are taken'} type={'text'} />
                        </div>
                        <div css={tw`mt-6 mx-2`}>
                            <Field
                                id={'dstdestination'}
                        ...
                        <div css={tw`mt-6 mx-2`}>
                            ...
                            <Switch name={'dstdestination'} onChange={() => setWipe(!wipe)} defaultChecked={wipe} />
                        </div>

                        <div css={tw`mt-6 text-right`}>
                            <Button type={'submit'}>Import from sftp</Button>
                        </div>
                    </Form>
                </Formik>
            </Modal>
            ...
            <TitledGreyBox title={'Server importer'} css={tw`relative`}>
                <p css={tw`text-sm`}>
                    ...
                    <strong css={tw`font-medium`}>When import start the server is stoped and are blocked during import.</strong>
                </p>
                <div css={tw`mt-6 text-right`}>
