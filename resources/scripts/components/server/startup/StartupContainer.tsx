...
import RedMLicenseKeyModal from '@/components/iceline/RedMLicenseKeyModal';
...
const StartupContainer = () => {
    ...
    const [showRedMModal, setShowRedMModal] = useState(false);
    ...
    return !data ? (
        ...
        <ServerContentBlock title={'Startup Settings'} showFlashKey={'startup:image'}>
            <div css={tw`flex`}>
                <TitledGreyBox title={'Startup Command'} css={tw`flex-1`}>
                    <div css={tw`px-1 py-2`}>
                        <p css={tw`font-mono bg-icelinebox-700 rounded py-2 px-4`}>{data.invocation}</p>
                    </div>
                </TitledGreyBox>
                    ...
                    {Object.keys(data.dockerImages).length > 1 && !isCustomImage ? (
                        <>
                            <InputSpinner visible={loading}>
                                <Select disabled={Object.keys(data.dockerImages).length < 2} onChange={updateSelectedDockerImage} defaultValue={variables.dockerImage}>
            ...
            {egg.name.includes('FiveM') && !egg.name.toLowerCase().includes('proxy') && (
                <>
                    <FiveMLicenseKeyModal visible={showFiveMModal} onDismissed={() => setShowFiveMModal(false)} />
                    <Button css={tw`mb-4`} onClick={() => setShowFiveMModal(true)}>
                        FiveM License Key Info
                    </Button>
                </>
            )}
            {egg.name.includes('RedM') && (
                <>
                    <RedMLicenseKeyModal visible={showRedMModal} onDismissed={() => setShowRedMModal(false)} />
                    <Button css={tw`mb-4`} onClick={() => setShowRedMModal(true)}>
                        RedM License Key Info
                    </Button>
                </>
            )}
            <div css={tw`grid gap-8 md:grid-cols-2`}>
                {data.variables
                    .sort((a: ServerEggVariable, b: ServerEggVariable) => {
