...
import SyncSubdomainButton from '@/components/server/subdomain/SyncSubdomainButton';
                        ...
                        <>
                            <div css={tw`w-full lg:w-8/12 mt-4 lg:mt-0`}>
                                ...
                                {data.subdomains.length < 1 ? (
                                    <p css={tw`text-center text-sm text-neutral-400 pt-4 pb-4`}>There are no subdomains for this server.</p>
                                ) : (
                                        ...
                                        <GreyRowBox $hoverable={false} css={tw`flex-wrap md:flex-nowrap mt-2`} key={key}>
                                            <div css={tw`flex items-center w-full md:w-auto`}>
                                                ...
                                                <div css={tw`mr-4 flex-1 md:w-64`}>
                                                    <Code>
                                                        ...
                                                        {item.record_type === 'CNAME' ? `:${item.port}` : ''}
                                                    </Code>
                                            ...
                                            <Can action={'subdomain.manage'}>
                                                <div css={tw`w-full md:flex-none md:w-40 md:text-center mt-4 md:mt-0 text-right ml-4`}>
                                                    <div css={tw`mr-4`}>
                                                        <SyncSubdomainButton subdomainId={item.id} onSynced={() => mutate()} />
                                                    </div>
                                                    {item.deletable > 0 && <DeleteSubdomainButton subdomainId={item.id} onDeleted={() => mutate()} />}
                                                </div>
                                            </Can>
                                        </GreyRowBox>
                                    ))
                            ...
                            <div css={tw`w-full lg:w-4/12 lg:pl-4`}>
                                <TitledGreyBox title={'Subdomain Help'}>
                                    <div css={tw`px-1 py-2`}>
                                        You can create Subdomain for your server. For example: <code>myserver.example.com</code> or <code>myserver.example.com:25565</code>
                                    </div>
                                </TitledGreyBox>
                            </div>
                        </>
