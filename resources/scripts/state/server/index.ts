...
function sentryServer(server: Server): any {
    return {
        id: server.id,
        ...
        isInstalling: server.status === 'installing',
...
const server: ServerDataStore = {
    ...
    setServer: action((state, payload) => {
        if (!isEqual(payload, state.data)) {
            state.data = payload;
            Sentry.setContext('server', sentryServer(payload));
        }
    }),
    ...
    setServerFromState: action((state, payload) => {
        ...
        if (!isEqual(output, state.data)) {
            state.data = output;
            Sentry.setContext('server', sentryServer(output));
        }
    }),
