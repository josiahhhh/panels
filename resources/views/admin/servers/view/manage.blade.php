    ...
    <div class="row">
        ...
        @else
            ...
            <div class="col-sm-4">
                <div class="box box-success">
                    <div class="box-header with-border">
                        <h3 class="box-title">Fix Stalled Transfer</h3>
                    </div>
                    <div class="box-body">
                        <div class="alert alert-danger">
                            ALWAYS check the server console for status updates for a couple of minutes BEFORE using this to ensure the transfer is actually stuck.
                        </div>
                        <p>
                            Attempts to fix the transfer status of a potentially stalled server transfer.

                            Does not work to stop a transfer in-progress and will damage servers if the following isn't checked first:
                            <ol>
                                <li>There is not a "failure to connect to panel/node" message in red at the top of the page on the server console.</li>
                                <li>Transfer has been running for more than an hour or so</li>
                                <li>Server console is not outputting transfer status messages every few minutes</li>
                            </ol>
                            This WILL break a server and need manual sysadmin correction if the above is not checked first.
                        </p>
                    </div>

                    <div class="box-footer">
                        <form action="{{ route('admin.servers.view.manage.fixTransfer', $server->id) }}" method="POST">
                            {!! csrf_field() !!}
                            <input type="hidden" name="action" value="unsuspend" />
                            <button type="submit" class="btn btn-success">Fix Transfer</button>
                        </form>
                    </div>
                </div>
            </div>
