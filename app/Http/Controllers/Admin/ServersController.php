...
class ServersController extends Controller
{
    ...
    /**
     * Attempts to automatically fix a stalled server transfer.
     *
     ...
     */
    public function fixStalledTransfer(Request $request, Server $server)
    {
        $roles = DB::table('permissions')->get();
        foreach ($roles as $role) {
            if ($role->id == Auth::user()->role) {
        ...
        // Check if there actually is a transfer to fix
        if (is_null($transfer)) {
            throw new DisplayException('This server has no currently active transfers.');
        }

        // Sanity check, don't allow using the button instantly
        if ($transfer->created_at->diffInMinutes() < 5) {
            throw new DisplayException('Please wait for more then 5 minutes before attempting to reset the transfer status.');
        }

        // Check the current server node to determine the transfer status
        if ($server->node_id == $transfer->new_node) {
            $transfer->successful = true;
            $this->alert->success("Transfer status reset, server has successfully been moved to new node!")->flash();
        } else {
            $transfer->successful = false;
            $this->alert->warning("Transfer status reset, server HAS NOT been transferred to it's new node. Transfer will need to be re-started if transferring to the new node is still desired.")->flash();
        }
    ...
    public function updateBuild(Request $request, Server $server)
    {
        ...
        try {
            $this->buildModificationService->handle($server, $request->only([
                ...
                'database_limit', 'allocation_limit', 'backup_limit', 'backup_limit_by_size',
                'oom_disabled',
            ]));
