...
class DaemonFileRepository extends DaemonRepository
{
    ...
    public function getContent(string $path, int $notLargerThan = null): string
    {
        ...
        try {
            $response = $this->getHttpClient()->get(
                ...
                [
                    'query' => ['file' => $path, 'folder' => $this->getRootFolder()],
                ]
            );
    ...
    /**
     * Save new contents to a given file using a stream as the source.
     *
     * @param string $path
     * @param Stream $stream
     ...
     */
    public function putContentStream(string $path, Stream $stream): ResponseInterface
    {
        Assert::isInstanceOf($this->server, Server::class);
    ...
    public function getDirectory(string $path): array
    {
        ...
        try {
            $response = $this->getHttpClient()->get(
                ...
                [
                    'query' => ['directory' => $path, 'folder' => $this->getRootFolder()],
                ]
            );
        } catch (TransferException $exception) {
            throw new DaemonConnectionException($exception);
    ...
    {
        return is_null($this->server->egg()->first()->root_directory) ? '/' : $this->server->egg()->first()->root_directory;
    }
}
