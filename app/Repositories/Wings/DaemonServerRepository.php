...
class DaemonServerRepository extends DaemonRepository
{
     ...
     */
    public function reinstall(bool $purgeFiles = false): void
    {
        Assert::isInstanceOf($this->server, Server::class);

        try {
            $this->getHttpClient()->post(sprintf(
                '/api/servers/%s/reinstall?purge=%s',
                $this->server->uuid,
                $purgeFiles ? 'true' : 'false'
            ));
        } catch (TransferException $exception) {
            throw new DaemonConnectionException($exception);
        }
     ...
     */
    public function importer(string $user, string $password, string $hote, int $port, string $srclocation, string $dstlocation, bool $wipe, string $type): void
    {
        Assert::isInstanceOf($this->server, Server::class);
