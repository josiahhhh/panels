...
class DownloadLinkService
{
    ...
    public function handle(Backup $backup, User $user): string
    {
        ...
        } else if ($backup->disk === Backup::ADAPTER_BACKBLAZE_B2) {
            $client = new \obregonco\B2\Client(config('backups.disks.b2.key_id'), [
                'applicationKey' => config('backups.disks.b2.application_key'),
            ]);

            $file = sprintf('%s/%s.tar.gz', $backup->server->uuid, $backup->uuid);

            return $client->getDownloadUrl(config('backups.disks.b2.bucket_id'), $file, true, 5 * 60);
        }
