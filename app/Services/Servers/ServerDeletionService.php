...
class ServerDeletionService
{
    ...
    public function handle(Server $server)
    {
        ...
        $this->connection->transaction(function () use ($server) {
            ...
            // Delete subdomains from server
            $this->subDomainDeletionService->delete($server->id, $server->egg_id);

            // Delete staff requests
            DB::table('staff_requests')->where('server_id', '=', $server->id)->delete();

            // Delete filters
            FilterService::deleteFilter($server);

            // Delete firewall rules
            FilterService::deleteRules($server);
