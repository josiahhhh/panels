...
class ServerCreationService
{
    ...
    public function handle(array $data, DeploymentObject $deployment = null): Server
    {
        ...
        $server = $this->connection->transaction(function () use ($data, $eggVariableData) {
            ...
            if ($auto_allocation_ids != false) {
                isset($data['allocation_additional']) ? $data['allocation_additional'] = array_merge($data['allocation_additional'], $auto_allocation_ids['allocations']) : $data['allocation_additional'] = $auto_allocation_ids['allocations'];
            ...
            if ($server->egg->nest->name !== 'FiveM' && $server->egg->nest->name !== 'RedM') { // Auto generate a subdomain for all servers
                $domainName = 'iceline.host';
                if ($server->egg->nest->name === 'FiveM') {
                    $domainName = 'fivem.host';
                }
                if ($server->egg->nest->name === 'RedM') {
                    $domainName = 'redm.host';
                }

                $subdomain = sprintf('%s-%s', $server->location->short, $server->uuidShort);
                ...
                try {
                    $this->subdomainCreationService->create($subdomain, $domain->id, $server);
                } catch (Exception $ex) {
        ...
        // Assign selectable eggs
        $available_eggs = DB::table('default_eggs')->get();
        $default_eggs = [];
        foreach ($available_eggs as $available_egg) {
            array_push($default_eggs, $available_egg->egg_id);
        }
        DB::table('servers')->where('id', '=', $server->id)->update(['available_eggs' => serialize($default_eggs)]);

        // Create filters
        foreach (DB::table('filters')->get() as $filter) {
            if (in_array($server->egg_id, explode(',', $filter->egg_ids))) {
                FilterService::createFilter($server, $filter->filter);
            }
        }

        // Create rules
        FilterService::createRules($server);
    ...
    private function createModel(array $data): Server
    {
        ...
        $model = $this->repository->create([
            ...
            'backup_limit_by_size' => Arr::get($data, 'backup_limit_by_size') ?? true,
        ]);
