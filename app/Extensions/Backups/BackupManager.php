...
class BackupManager
{
    ...
    /**
     * Creates a new B2 adapter.
     *
     * @param array $config
     * @throws \Exception
     * @return \League\Flysystem\AdapterInterface
     */
    public function createB2Adapter (array $config) {
        $client = new Client($config['key_id'], $config['application_key'], [
            'client' => new HttpClient(['exceptions' => true])
        ]);

        return new BackblazeAdapter($client, $config['bucket_name']);
    }
